import type { LegalBusiness } from '../Legal.types';

const disallowedConductItems = (businessName: LegalBusiness['name']) => ([
...
const conductItemsString = (businessName: LegalBusiness['name']) => {
	const items = disallowedConductItems(businessName) as string[];
	const last = items.length - 1;
	return items.reduce((acc, item, idx) =>
		idx === last ? acc + `or ${item}.` : acc + `${item}; `, '');
};
...
const SiteDisclosures = {
	...
	dataSecurityNotice: (siteName: LegalBusiness['siteName']) => {
		return `Our servers are protected physically and electronically. Any connection between you and ${siteName} is protected by 256-bit SSL encryption.`;
	},

	disclaimer: (businessName: LegalBusiness['name']) => {
		return `The information contained in this website is for general information purposes only. The information is provided by ${businessName} and while we endeavor to keep the information up to date and correct, we make no representations or warranties of any kind, express or implied, about the completeness, accuracy, reliability, suitability or availability with respect to the website or the information, products, services, or related graphics contained on the website for any purpose. Any reliance you place on such information is therefore strictly at your own risk. In no event will we be liable for any loss or damage including without limitation, indirect or consequential loss or damage, or any loss or damage whatsoever arising from loss of data or profits arising out of, or in connection with, the use of this website. Through this website you are able to link to other websites which are not under the control of ${businessName}. We have no control over the nature, content and availability of those sites. The inclusion of any links does not necessarily imply a recommendation or endorse the views expressed within them. Every effort is made to keep the website up and running smoothly. However, ${businessName} takes no responsibility for, and will not be liable for, the website being temporarily unavailable due to technical issues beyond our control.`;
	},

	copyRight: (businessName: LegalBusiness['name']) => {
		return `© ${new Date().getFullYear()} ${businessName}. All rights reserved.`;
	},

	statementOfGoverningLaw: (businessName: LegalBusiness['name']) => {
		return `THIS AGREEMENT WILL BE GOVERNED BY AND CONSTRUED IN ACCORDANCE WITH THE LAWS OF THE STATE OF TEXAS, IRRESPECTIVE OF THE CONFLICT OF LAWS RULES. FURTHERMORE, BECAUSE YOU AND ${businessName?.toUpperCase() || '[BUSINESS NAME]'} AGREE THAT THE TEXAS COURTS ARE THE EXCLUSIVE FORUM FOR RESOLVING ANY DISPUTES ARISING OUT OF THIS AGREEMENT, THE PARTIES SUBMIT THEMSELVES TO THE PERSONAL JURISDICTION OF THE TEXAS COURTS.`;
	},
	termsOfUse: (siteName: LegalBusiness['siteName'], business: LegalBusiness) => ([
		{
			_id: '1',
			title: '1. Acceptance of Terms',
			content: `By using ${siteName}, creating, or accessing your ${siteName} account, including by signing in with a third-party service or partner (such as Google, Yahoo, ADP or RBC), or by otherwise using the Services we offer, you are agreeing to be bound by the Agreement without any modification or qualification. IF YOU ARE DISSATISFIED WITH THE AGREEMENT, OUR RULES, POLICIES, GUIDELINES OR PRACTICES, OR OUR OPERATION OF THE ${siteName?.toUpperCase() || '[SITE NAME]'} WEBSITE OR THE SERVICES, YOUR SOLE AND EXCLUSIVE REMEDY IS TO DISCONTINUE USING THE ${siteName?.toUpperCase() || '[SITE NAME]'} WEBSITE AND/OR OUR SERVICES, UNLESS ANOTHER REMEDY IS EXPRESSLY SET OUT IN THIS AGREEMENT. If for any reason you are unable to meet all the conditions set forth in this Agreement or if you breach this Agreement, your permission to access or use our Services, any materials downloaded or printed by you, and ${siteName?.toUpperCase() || '[SITE NAME]'} immediately lapses.`,
		},
		{
			_id: '2',
		...
		},
		{
			...
			content: `THESE TERMS OF USE WILL BE GOVERNED BY AND CONSTRUED IN ACCORDANCE WITH THE LAWS OF THE STATE OF TEXAS, AND ANY DISPUTES ARISING FROM THESE TERMS OF USE OR ${siteName?.toUpperCase() || '[SITE NAME]'} WILL BE RESOLVED IN THE COURTS OF THE STATE OF TEXAS`,
		},
		{
			_id: '16',
			title: '16. Contact Us',
	...
	privacyPolicy: (siteName: LegalBusiness['siteName'], business: LegalBusiness) => ([
		{
			_id: '1',
			title: '1. Introduction',
			content: `This Privacy Policy governs the manner in which ${business.name?.toUpperCase() || '[BUSINESS NAME]'} collects, uses, maintains and discloses information collected from users (each, a “User”) of the ${siteName?.toUpperCase() || '[SITE NAME]'} website (“Site”). This privacy policy applies to the Site and all products and services offered by ${business.name?.toUpperCase() || '[BUSINESS NAME]'}.`,
		},
		{
			_id: '2',
		...
		{
			_id: '14',
			title: `14. This document was last updated on ${business.date}`
		},
	])
}
